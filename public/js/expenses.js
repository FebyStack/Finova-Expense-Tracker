...
import { loadCategories, bgFromColor, getCategoryStyle } from './categories.js';
...
export async function openExpenseModal(prefill = {}) {
  ...
  if (prefill.currency) {
    ...
    if (symEl) symEl.textContent = getCurrencySymbol(prefill.currency);
  }
...
// Toast is imported from toast.js
...
window.applyQuickFill = function(source, index) {
  ...
  if (data.currency) {
    ...
    const symEl = document.getElementById('expCurrencySymbol');
    if (symEl) symEl.textContent = getCurrencySymbol(data.currency);
  }
