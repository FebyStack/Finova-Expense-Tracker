...
import { convertItems, formatCurrency, warmRateCache, convertSync, formatDate } from './currency.js';
import { getCategoryStyle } from './categories.js';
    ...
    export async function loadDashboard(user, userData) {
      // userData comes from api/me (Node Backend), which uses snake_case
      const name = userData?.display_name || user.display_name || userData?.email || user.email;
      setGreeting(name);
      setMonthLabel();

      const uid      = user.id || user.uid;
      const currency = userData?.base_currency || user.base_currency || 'PHP';
      const now      = new Date();
    ...
    async function loadBudgetOverview(uid, currency, now, convertedExpenses = []) {
      ...
      try {
        ...
        container.innerHTML = budgets.map(b => {
          ...
          const rawLim = parseFloat(b.limit_amount || b.amount || b.limitAmount) || 1;
    ...
    async function loadAIInsights(uid, currency, force = false) {
      ...
      if (!navigator.onLine) {
        container.innerHTML = `
          <div class="ai-offline-placeholder">
            <i class="fa-solid fa-cloud-slash"></i>
            <div class="placeholder-title">Offline Mode</div>
            <div class="placeholder-text">AI Insights require an internet connection. They will automatically refresh once you're back online.</div>
          </div>
        `;
    ...
    window.addEventListener('online', () => {
      const user = window.currentUser;
      if (user) loadAIInsights(user.uid, null, false);
    });
