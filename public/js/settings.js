...
async function saveBaseCurrency() {
  ...
  try {
    // Update PostgreSQL
    await updateUser(user.id, { baseCurrency: newCurrency });
    ...
    showToast(`Base currency changed to ${newCurrency}`);
...
async function saveCategoryFromForm() {
  ...
  try {
    ...
    closeCategoryForm();
    renderCategoryList();
    showToast(editingIndex !== null ? 'Category updated!' : 'Category added!');
  } catch (err) {
...
// Toast is imported from shared toast.js
...
export async function initSettings() {
  ...
  document.getElementById('btnManualSyncRetry')?.addEventListener('click', () => {
    if (window.manualSync) window.manualSync();
  });
}
