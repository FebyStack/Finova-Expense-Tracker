...
export async function fetchRates(baseCurrency = 'PHP') {
  ...
  // Return cached if fresh OR if offline (to bypass network timeouts)
  const isOffline = !navigator.onLine;
  if (isOffline || (rateCache[baseCurrency] && lastFetch[baseCurrency] && (now - lastFetch[baseCurrency] < CACHE_TTL))) {
    if (isOffline) console.log(`ℹ️ [Currency] Offline: Using cached rates for ${baseCurrency}`);
    return rateCache[baseCurrency] || null;
  }

  try {
    // 1. Race fetch against a 3-second timeout
    const controller = new AbortController();
    const id = setTimeout(() => controller.abort(), 3000);

    const res = await fetch(`${API_BASE}/${baseCurrency}`, { signal: controller.signal });
    clearTimeout(id);
    ...
    console.warn(`⚠️ Exchange rate fetch failed for ${baseCurrency}:`, err.message);
    // Return whatever we have cached, or null
    return rateCache[baseCurrency] || null;
  }
}
